...
async function activateChallenge(payment) {
  ...
  try{const {data:aff}=await supabase.from('affiliates').select('*').eq('referred_id',profile.user_id).eq('status','pending').single();if(aff)await supabase.from('affiliates').update({status:'approved'}).eq('id',aff.id);}catch(e){}
...
app.post('/track-referral',async(req,res)=>{
  const{referrer_code,referred_user_id}=req.body;if(!referrer_code||!referred_user_id)return res.status(400).json({error:'Missing fields'});
  try{
    let referrer=null;
    const{data:r1}=await supabase.from('profiles').select('user_id').eq('user_id',referrer_code).single().catch(()=>({data:null}));if(r1)referrer=r1;
    if(!referrer){const{data:r2}=await supabase.from('profiles').select('user_id').eq('referral_code',referrer_code).single().catch(()=>({data:null}));if(r2)referrer=r2;}
    if(!referrer)return res.status(404).json({error:'Referrer not found'});
    const{data:existing}=await supabase.from('affiliates').select('id').eq('referrer_id',referrer.user_id).eq('referred_id',referred_user_id).single().catch(()=>({data:null}));
    if(existing)return res.json({success:true,message:'Already tracked'});
    await supabase.from('affiliates').insert({referrer_id:referrer.user_id,referred_id:referred_user_id,status:'pending',commission:50,paid:false});
    res.json({success:true});
  }catch(err){res.status(500).json({error:err.message});}
});
...
app.post('/submit-affiliate-wallet',async(req,res)=>{
  const{user_id,wallet}=req.body;if(!user_id||!wallet)return res.status(400).json({error:'Missing fields'});
  try{await supabase.from('profiles').update({affiliate_wallet:wallet}).eq('id',user_id);res.json({success:true});}catch(err){res.status(500).json({error:err.message});}
});
